...
def get_mutations(mutation_file: str, target_seq: str = "", mutant_delim=":") -> pd.DataFrame:
    ...
    if "mutated_sequence" not in mutations.columns:
        ...
        if "mutant" not in mutations.columns:
            ...
            raise ValueError(
                "mutated_sequence column must be present if mutant column is not")
    return mutations
    ...
    for mutant in mutants:
        if is_indel(mutant):
            ...
            for indiv_mutant in indiv_mutants:
                ...
                if not orig_aa == target_seq[pos-1]:
                    raise ValueError(
                        f"original amino acid {orig_aa} at position {pos} does not match target sequence amino acid {target_seq[pos-1]}")
                pos = int(pos)
...
def replace_three_letter_aas(mutant: str) -> str:
    ...
    """
    for key, value in SeqUtils.IUPACData.protein_leters_3to1.items():
