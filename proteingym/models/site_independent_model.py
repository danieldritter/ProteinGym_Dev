...
class SiteIndependentModel(AlignmentProbabilityModel):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if "alignment_kwargs" in kwargs:
            kwargs["alignment_kwargs"]["uppercase"] = True
        else:
            kwargs["alignment_kwargs"] = {"uppercase": True}
        super().__init__(model_checkpoint=None, **kwargs)
        self.pseudocount = pseudocount
        self.include_gaps = include_gaps
        if include_gaps:
            self.aa_dict = AA_INTEGER_MAP.update({"-": len(AA_INTEGER_MAP)})
        else:
        ...
        if isinstance(pseudocount, np.ndarray):
            assert pseudocount.shape == (
                len(list(self.aa_dict.keys())),
                self.alignment_length,
            ), "Pseudocount array must be the same shape as the alignment sequences"
        self.log_frequencies = np.log(self.compute_frequencies())  # type: ignore
    ...
    def compute_frequencies(self) -> np.ndarray:
        ...
        if isinstance(self.pseudocount, int):
            frequencies = (
                np.ones((len(list(self.aa_dict.keys())), self.alignment_length))
                * self.pseudocount
            )
        else:
    ...
    def predict_logprobs(
        ...
        if wt_sequence is not None:
            assert (
                len(wt_sequence) == self.alignment_length
        ...
        for sequence in sequences:
            assert (
                len(sequence) == self.alignment_length
            ), "Sequences must be the same length as the alignment"
            seq_indices = [self.aa_dict[aa] for aa in sequence]
            logprob = np.sum(self.log_frequencies[seq_indices, range(len(sequence))])
