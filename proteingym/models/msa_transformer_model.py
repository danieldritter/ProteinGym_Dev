...
@AlignmentModel.register("msa_transformer_model")
class MSATransformerModel(AlignmentModel, ProteinLanguageModel):

    def __init__(self, random_seed: int = 0, num_msa_samples: int = 400, sampling_strategy: str = "random", **kwargs):
        kwargs_parsed = self.parse_config(kwargs)
        AlignmentModel.__init__(self, **kwargs_parsed)
        ProteinLanguageModel.__init__(self, **kwargs_parsed)
        ...
        self.processed_msa = self.read_msa(
            self.num_msa_samples, self.sampling_strategy)
    ...
    def read_msa(self, nseqs: int, sampling_strategy: str) -> List[Tuple[str, str]]:
        ...
        # TODO: Can probably optimize this to avoid making copies of lots of sequences
        ...
        if sampling_strategy == 'first_x_rows':
            msa = [
                ...
                for record in itertools.islice(SeqIO.parse(self.alignment_file, "fasta"), nseqs)
            ]
        elif sampling_strategy == 'random':
            msa = [
                (record.description, str(record.seq)) for record in SeqIO.parse(self.alignment_file, "fasta")
            ]
            nseqs = min(len(msa), nseqs)
            msa = random.sample(msa, nseqs)
        elif sampling_strategy == 'sequence-reweighting':
            assert self.alignment.weights_file is not None, "Weights file must be provided for sequence-reweighting"
            all_sequences_msa = []
            weights = []
            msa = []
                ...
                if seq_name == self.alignment.focus_seq_id:
                    ...
                    if seq_name in self.alignment.seq_name_to_weights:
                        all_sequences_msa.append(
                            (seq_name, self.alignment.seq_name_to_sequences[seq_name]))
                        weights.append(
                            self.alignment.seq_name_to_weights[seq_name])
            if len(all_sequences_msa) > 0:
                weights = np.array(
                    weights) / np.array(list(self.alignment.seq_name_to_weights.values())).sum()
                msa.extend(random.choices(
                    all_sequences_msa, weights=weights, k=nseqs-1))
        ...
        for i in range(batch_tokens.size(2)):
            ...
            if batch_tokens.size(-1) > 1024:
                ...
                start, end = get_optimal_window(
                    mutation_position_relative=i, seq_len_wo_special=len(wt_sequence)+2, model_window=1024)
                print("Start index {} - end index {}".format(start, end))
                batch_tokens_masked = large_batch_tokens_masked[:,
                                                                :, start:end]
            else:
                start = 0
            with torch.no_grad():
            ...
            if not self.nogpu and torch.cuda.is_available():
                all_token_probs.append(
                    token_probs[:, 0, i-start].detach().cpu())  # vocab size
            else:
    ...
    def predict_logprobs(self, sequences: List[str], wt_sequence: Union[str, None] = None):
