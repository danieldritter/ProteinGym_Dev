...
class ESMModel(ProteinLanguageModel):
    ...
    def predict_logprobs(self, sequences: List[str], wt_sequence: Union[str, None] = None) -> List[float]:
        ...
        if self.scoring_strategy != "pseudo-ppl":
            assert all(len(sequence) == len(wt_sequence)
                       for sequence in sequences), "Some mutated sequences are of different length from the wild type. Only substitutions are allowed for scoring strategies other than pseudo-ppl"
        data = [
        ...
        if self.scoring_strategy == "wt-marginals":
            ...
            scores = self.apply_wt_probabilities(
                sequences, wt_sequence, token_probs)
            ...
            raise ValueError(
                f"Scoring strategy {self.scoring_strategy} not supported")
        return scores
    ...
    def wt_marginals(self, batch_tokens: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            if batch_tokens.size(1) > self.model_window and self.scoring_window == "overlapping":
                ...
                token_probs = torch.zeros(
                    (batch_size, seq_len, len(self.alphabet))).cuda()
                ...
                start_right_window = (batch_tokens.size(
                    1) - 1) - 1024 + 1  # Last index is len-1
                end_right_window = batch_tokens.size(1) - 1
                while True:
                    # Left window update
                    left_window_probs = torch.log_softmax(self.model(
                        batch_tokens[:, start_left_window:end_left_window+1].cuda())["logits"], dim=-1)
                    token_probs[:, start_left_window:end_left_window +
                                1] += left_window_probs * weights.view(-1, 1)
                    token_weights[:,
                                  start_left_window:end_left_window+1] += weights
                    # Right window update
                    right_window_probs = torch.log_softmax(self.model(
                        batch_tokens[:, start_right_window:end_right_window+1].cuda())["logits"], dim=-1)
                    token_probs[:, start_right_window:end_right_window +
                                1] += right_window_probs * weights.view(-1, 1)
                    token_weights[:,
                                  start_right_window:end_right_window+1] += weights
                ...
                if final_overlap < 511:
                    ...
                    central_window_probs = torch.log_softmax(self.model(
                        batch_tokens[:, start_central_window:end_central_window+1].cuda())["logits"], dim=-1)
                    token_probs[:, start_central_window:end_central_window +
                                1] += central_window_probs * weights.view(-1, 1)
                    token_weights[:,
                                  start_central_window:end_central_window+1] += weights
                # Weight normalization
                token_probs = token_probs / \
                    token_weights.view(-1, 1)  # Add 1 to broadcast
            else:
    ...
    def masked_marginals(self, batch_tokens: torch.Tensor, wt_sequence: str) -> torch.Tensor:
        ...
        for i in range(batch_tokens.size(1)):
            ...
            if batch_tokens.size(1) > self.model_window and self.scoring_window == "optimal":
                ...
                start, end = get_optimal_window(mutation_position_relative=i, seq_len_wo_special=len(
                    wt_sequence)+2, model_window=self.model_window)
                batch_tokens_masked = large_batch_tokens_masked[:, start:end]
            elif batch_tokens.size(1) > self.model_window and self.scoring_window == "overlapping":
        ...
        for sequence in tqdm(sequences):
            ...
            for i in range(1, len(sequence) - 1):
                ...
                with torch.no_grad():
                    token_probs = torch.log_softmax(
                        self.model(batch_tokens_masked.cuda())["logits"], dim=-1)
                log_probs.append(token_probs[0, i, self.alphabet.get_idx(
                    sequence[i])].item())  # vocab size
    ...
    def apply_wt_probabilities(self, sequences: List[str], wt_sequence: str, token_probs: torch.Tensor) -> List[float]:
        ...
        for seq in sequences:
            ...
            for i, aa in enumerate(seq):
                ...
                if aa == wt_sequence[i]:
                    continue
                score += (token_probs[0, 1 + i, self.alphabet.get_idx(
                    aa)] - token_probs[0, 1 + i, self.alphabet.get_idx(wt_sequence[i])]).item()
            scores.append(score)
        return scores
