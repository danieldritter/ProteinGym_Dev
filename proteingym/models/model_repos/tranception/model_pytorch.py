...
def get_slopes(n, mode="standard_alibi", verbose=False):
    ...
    if math.log2(n).is_integer():
        ...
        result = get_slopes_power_of_2(
            closest_power_of_2) + get_slopes(2*closest_power_of_2)[0::2][:n-closest_power_of_2]
    if mode == "grouped_alibi":
        result = result * 4
        if verbose:
...
class SpatialDepthWiseConvolution(nn.Module):
    def __init__(self, head_dim: int, kernel_size: int = 3):
        ...
        self.conv = nn.Conv1d(in_channels=head_dim, out_channels=head_dim, kernel_size=(
            kernel_size,), padding=(kernel_size - 1,), groups=head_dim)
    ...
    def _attn(self, query, key, value, attention_mask=None, head_mask=None, alibi_bias=None):
        ...
        if alibi_bias is not None:
            attn_weights = attn_weights + \
                alibi_bias[:, :, :attn_weights.size(-1)]
    ...
    def forward(
        ...
        if self.attention_mode == "tranception":
            ...
            for kernel_idx in range(3):
                query_list.append(self.query_depthwiseconv[str(kernel_idx)](
                    query[:, (kernel_idx+1)*self.num_heads_per_kernel_size:(kernel_idx+2)*self.num_heads_per_kernel_size, :, :]))
                key_list.append(self.key_depthwiseconv[str(kernel_idx)](
                    key[:, (kernel_idx+1)*self.num_heads_per_kernel_size:(kernel_idx+2)*self.num_heads_per_kernel_size, :, :]))
                value_list.append(self.value_depthwiseconv[str(kernel_idx)](
                    value[:, (kernel_idx+1)*self.num_heads_per_kernel_size:(kernel_idx+2)*self.num_heads_per_kernel_size, :, :]))
...
class TranceptionBlock(nn.Module):
    def __init__(self, config, SDWC_kernel_size=None):
        ...
        self.attn = TranceptionBlockAttention(
            config, SDWC_kernel_size=SDWC_kernel_size)
...
class TranceptionModel(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.position_embedding = config.position_embedding if hasattr(
            config, "position_embedding") else "learned"
        if self.position_embedding == "learned":
            ...
            self.slopes = torch.Tensor(get_slopes(
                attn_heads, mode=self.position_embedding))
            # The softmax operation is invariant to translation, and bias functions used are always linear.
            alibi = self.slopes.unsqueeze(1).unsqueeze(
                1) * torch.arange(maxpos).unsqueeze(0).unsqueeze(0).expand(attn_heads, -1, -1)
            alibi = alibi.view(attn_heads, 1, maxpos)
            self.register_buffer('alibi', alibi)

        self.drop = nn.Dropout(config.embd_pdrop)
        self.h = nn.ModuleList([TranceptionBlock(config)
    ...
    def parallelize(self, device_map=None, num_cores=None):
        ...
        ) else device_prefix + str(min(self.device_map.keys()))
...
class TranceptionLMHeadModel(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.default_model_device = torch.device(
        ...
        if self.retrieval_aggregation_mode is not None:
            ...
            self.retrieval_inference_weight_LR = config.retrieval_inference_weight if hasattr(
                config, "retrieval_inference_weight") else 0.6
            self.retrieval_inference_weight_RL = config.retrieval_inference_weight if hasattr(
                config, "retrieval_inference_weight") else 0.6
            self.MSA_start = config.MSA_start
            self.MSA_end = config.MSA_end
            self.full_protein_length = config.full_protein_length if hasattr(
    ...
    def forward(
        ...
        if self.model_parallel:
            ...
            self.MSA_log_prior = self.MSA_log_prior.to(
        ...
        if labels is not None:
            ...
            if self.retrieval_aggregation_mode is not None:
                ...
                if self.retrieval_aggregation_mode == "aggregate_indel":
                    ...
                    # shift_logits only has one extra token compared to truncated_sequence_text (the BOS token)
                    if len(truncated_sequence_text) != shift_logits.shape[1]-1:
                        print("Tokenization error -- seq length: {} and shift_logits length - 1 : {}".format(
                            len(mutated_sequence), shift_logits.shape[1]-1))
                    MSA_log_prior, MSA_start, MSA_end = msa_utils.update_retrieved_MSA_log_prior_indel(
                        self, self.MSA_log_prior, self.MSA_start, self.MSA_end, mutated_sequence[0], self.clustal_hash)
                ...
                if flip is None:
                    flip = torch.zeros(batch_size).to(
                        fused_shift_log_probas.device)
                flip = flip > 0
                ...
                for seq_index in range(batch_size):
                    ...
                    if max_prior_slice <= min_prior_slice:
                        print("Non overlapping region detected: min_prior_slice {} and max_prior_slice {}".format(
                            min_prior_slice, max_prior_slice))
                        print(f"TMP lood mutated_sequence={mutated_sequence}")
                        print(
                            f"TMP lood seq_index={seq_index}, shift_labels={shift_labels[seq_index]}, {input_ids=}, {return_dict=}")
                        continue

                    slice_prior = MSA_log_prior[min_prior_slice:max_prior_slice, :].to(
                        fused_shift_log_probas.device)
                    if flip[seq_index]:
                        slice_prior = torch.flip(slice_prior, dims=(0,))
                        min_logits_slice = max(0, end_slice[seq_index]-MSA_end)
                        max_logits_slice = min_logits_slice + \
                            (max_prior_slice-min_prior_slice)
                        fused_shift_log_probas[seq_index, min_logits_slice:max_logits_slice, :] = (
                            1-self.retrieval_inference_weight_RL)*shift_log_probas[seq_index, min_logits_slice:max_logits_slice, :] + self.retrieval_inference_weight_RL*slice_prior
                    else:
                        min_logits_slice = max(
                            0, MSA_start-start_slice[seq_index])
                        max_logits_slice = min_logits_slice + \
                            (max_prior_slice-min_prior_slice)
                        fused_shift_log_probas[seq_index, min_logits_slice:max_logits_slice, :] = (
                            1-self.retrieval_inference_weight_LR)*shift_log_probas[seq_index, min_logits_slice:max_logits_slice, :] + self.retrieval_inference_weight_LR*slice_prior
                ...
                if self.retrieval_aggregation_mode == "aggregate_indel":
                    try:
                        ...
                        inserted_retrieval_positions = [True if slice_prior[i].sum() == 0 else False for i in range(
                            len(slice_prior))]+[True]  # Last True is for the end of sentence token
                        fused_shift_log_probas[:, inserted_retrieval_positions,
                                               :] = shift_log_probas[:, inserted_retrieval_positions, :]
                ...
                loss = loss_fct(input=fused_shift_log_probas.view(-1, fused_shift_log_probas.size(-1)),
                                target=shift_labels.view(-1)).view(fused_shift_log_probas.shape[0], fused_shift_log_probas.shape[1])
    ...
    def score_mutants(self, DMS_data, target_seq=None, scoring_mirror=True, batch_size_inference=10, num_workers=10, indel_mode=False):
        ...
        assert (
            'mutated_sequence' in df), "DMS file to score does not have mutated_sequence column"
        if 'mutant' not in df:
            # if mutant not in DMS file we default to mutated_sequence
            df['mutant'] = df['mutated_sequence']
        ...
        if target_seq is not None:
            df_left_to_right_slices = scoring_utils.get_sequence_slices(
                df, target_seq=target_seq, model_context_len=self.config.n_ctx - 2, indel_mode=indel_mode, scoring_window=self.config.scoring_window)
        else:
            df_left_to_right_slices = scoring_utils.get_sequence_slices(df, target_seq=list(df['mutated_sequence'])[
                                                                        0], model_context_len=self.config.n_ctx - 2, indel_mode=indel_mode, scoring_window='sliding')
        print("Scoring sequences from left to right")
        scores_L_to_R = scoring_utils.get_tranception_scores_mutated_sequences(
            model=self, mutated_sequence_df=df_left_to_right_slices, batch_size_inference=batch_size_inference, score_var_name='avg_score_L_to_R', target_seq=target_seq, num_workers=num_workers, indel_mode=indel_mode)
        if scoring_mirror:
            print("Scoring sequences from right to left")
            df_right_to_left_slices = df_left_to_right_slices.copy()
            df_right_to_left_slices['sliced_mutated_sequence'] = df_right_to_left_slices['sliced_mutated_sequence'].apply(
                lambda x: x[::-1])
            scores_R_to_L = scoring_utils.get_tranception_scores_mutated_sequences(
                model=self, mutated_sequence_df=df_right_to_left_slices, batch_size_inference=batch_size_inference, score_var_name='avg_score_R_to_L', target_seq=target_seq, num_workers=num_workers, reverse=True, indel_mode=indel_mode)
            all_scores = pd.merge(scores_L_to_R, scores_R_to_L,
                                  on='mutated_sequence', how='left', suffixes=('', '_R_to_L'))
            all_scores['avg_score'] = (
                all_scores['avg_score_L_to_R'] + all_scores['avg_score_R_to_L']) / 2.0
        else:
        ...
        if target_seq in DMS_data[mutant_column].values:
            if scoring_mirror:
                wt_row = pd.DataFrame([[target_seq, 0, 0, 0]], columns=[
                                      mutant_column, 'avg_score_L_to_R', 'avg_score_R_to_L', 'avg_score'])
            else:
                wt_row = pd.DataFrame([[target_seq, 0, 0]], columns=[
                                      mutant_column, 'avg_score_L_to_R', 'avg_score'])
    ...
    def encode_batch(self, protein_sequence, sequence_name="sliced_mutated_sequence"):
        ...
        """
        protein_sequence[sequence_name] = scoring_utils.sequence_replace(
            sequences=protein_sequence[sequence_name], char_to_replace='X', char_replacements='ACDEFGHIKLMNPQRSTVWY')
        protein_sequence[sequence_name] = scoring_utils.sequence_replace(
            sequences=protein_sequence[sequence_name], char_to_replace='B', char_replacements='DN')
        protein_sequence[sequence_name] = scoring_utils.sequence_replace(
            sequences=protein_sequence[sequence_name], char_to_replace='J', char_replacements='IL')
        protein_sequence[sequence_name] = scoring_utils.sequence_replace(
            sequences=protein_sequence[sequence_name], char_to_replace='Z', char_replacements='EQ')
