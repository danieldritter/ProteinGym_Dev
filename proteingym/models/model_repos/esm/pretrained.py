...
def _load_model_and_alphabet_core_v1(model_data):
    ...
    if model_data["args"].arch == "roberta_large":
        ...
        model_args["emb_layer_norm_before"] = has_emb_layer_norm_before(
            model_state)
...
def _load_model_and_alphabet_core_v2(model_data):
    def upgrade_state_dict(state_dict):
        ...
        state_dict = {pattern.sub("", name): param for name,
...
def load_model_and_alphabet_core(model_name, model_data, regression_data=None):
    ...
    if model_name.startswith("esm2"):
        model, alphabet, model_state = _load_model_and_alphabet_core_v2(
            model_data)
    else:
        model, alphabet, model_state = _load_model_and_alphabet_core_v1(
            model_data)
    ...
    if regression_data is None:
        expected_missing = {"contact_head.regression.weight",
