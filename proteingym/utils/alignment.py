...
from proteingym.constants.data import (AA_ALPHABET_ALL, AA_ALPHABET_UPPER,
                                       AA_INTEGER_MAP, AA_INTEGER_MAP_ALL)
...
class Alignment:
    ...
    def __init__(
        self,
        alignment_file: str,
        weights_file: Union[str, None] = None,
        ...
        self.focus_seq_id, self.focus_seq, self.seq_name_to_sequences = (
            self.read_alignment(alignment_file, alignment_format, uppercase)
        )
        ...
        self.remove_sequences_with_indeterminate_aas = (
            remove_sequences_with_indeterminate_aas
        )
        ...
        if remove_sequences_with_indeterminate_aas:
            ...
            self.seq_name_to_sequences = {
                ...
                if set(seq).issubset(set(list(self.alphabet) + ["-", "."]))
            }
            print(
        ...
        if self.weights_file is not None:
            if os.path.exists(self.weights_file):
                ...
                print(f"Computing sequence weights and saving to {self.weights_file}")
    ...
    def read_alignment(
        self, alignment_file: str, alignment_format: str, uppercase: bool = True
    ) -> Tuple[Union[str, None], Union[str, None], dict[str, str]]:
    ...
    def get_focus_alignment(
        self, alignment: dict[str, str], focus_cols: List[int]
    ) -> dict[str, str]:
    ...
    def filter_sequence_fragments(
        self, alignment: dict[str, str], perc_max_gap: float = 1.0
    ) -> dict[str, str]:
    ...
    def one_hot_encode_alignment(self, alignment: dict[str, str]) -> np.ndarray:
        ...
        one_hot_encoding = np.zeros(
            (
                len(alignment),
                len(alignment[list(alignment.keys())[0]]),
                len(self.alphabet),
            )
        )
